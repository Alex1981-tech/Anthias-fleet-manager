...
const CMD_GROUPS: CmdGroup[] = [
  {
    ...
    commands: [
      ...
      { label: 'logs server', cmd: 'cd ~/screenly && docker compose logs --tail 50 anthias-server', desc: 'last 50' },
      { label: 'logs viewer', cmd: 'cd ~/screenly && docker compose logs --tail 50 anthias-viewer', desc: 'last 50' },
      { label: 'logs nginx', cmd: 'cd ~/screenly && docker compose logs --tail 30 anthias-nginx', desc: 'last 30' },
      { label: 'restart all', cmd: 'cd ~/screenly && docker compose restart' },
      { label: 'restart server', cmd: 'cd ~/screenly && docker compose restart anthias-server' },
      { label: 'restart viewer', cmd: 'cd ~/screenly && docker compose restart anthias-viewer' },
